...
void get_modules_list(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const auto modules_path{ fs::path(emuenv.get_wide_pref_path()) / "vs0/sys/external/" };
...
static void reset_emulator(GuiState &gui, EmuEnvState &emuenv) {
    ...
    emuenv.cfg.pref_path = emuenv.get_pref_path();
...
static void change_emulator_path(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (result == host::dialog::filesystem::Result::SUCCESS && emulator_path.wstring() != emuenv.get_wide_pref_path()) {
        ...
        LOG_INFO("Successfully moved Vita3K path to: {}", emuenv.get_pref_path());
    }
}
...
static void save_config(GuiState &gui, EmuEnvState &emuenv) {
    if (gui.configuration_menu.custom_settings_dialog) {
        const auto CONFIG_PATH{ fs::path(emuenv.get_wide_pref_path()) / "config" };
...
void draw_settings_dialog(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (ImGui::BeginTabItem("GPU")) {
        ...
        const auto shaders_cache_path{ fs::path(emuenv.cache_path) / "cache/shaders" };
        if (fs::exists(shaders_cache_path) && !fs::is_empty(shaders_cache_path)) {
            ...
            if (ImGui::Button("Clean Shaders Cache and Log")) {
                ...
                fs::remove_all(fs::path(emuenv.cache_path) / "shaderlog");
            }
        }
        ImGui::EndTabItem();
    } else
    ...
    if (ImGui::BeginTabItem("Emulator")) {
        ...
        if (emuenv.cfg.pref_path != emuenv.default_path) {
            ...
            if (ImGui::Button("Reset Emulator Path")) {
                if (emuenv.default_path != emuenv.get_pref_path()) {
                    emuenv.set_pref_path(string_utils::utf_to_wide(emuenv.default_path));

                    // Refresh the working paths
                    reset_emulator(gui, emuenv);
                    LOG_INFO("Successfully restored default path for Vita3K files to: {}", emuenv.get_pref_path());
                }
            }
            if (ImGui::IsItemHovered())
        ...
        if (ImGui::Button("Clear Custom Config")) {
            if (fs::remove_all({ fs::path(emuenv.get_wide_pref_path()) / "config" })) {
