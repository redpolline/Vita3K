...
void init_live_area(GuiState &gui, EmuEnvState &emuenv, const std::string &app_path) {
    ...
    if (!gui.live_area_contents.contains(app_path)) {
        ...
        const auto APP_PATH{ fs::path(emuenv.get_wide_pref_path()) / app_device._to_string() / "app" / app_path };
        ...
        if (doc.load_file(template_xml.c_str())) {
            ...
            for (const auto &contents : name) {
                ...
                if (default_contents)
                    vfs::read_file(VitaIoDevice::vs0, buffer, emuenv.get_wide_pref_path(), "data/internal/livearea/default/sce_sys/livearea/contents/" + contents.second);
                else if (app_device == VitaIoDevice::vs0)
                    vfs::read_file(VitaIoDevice::vs0, buffer, emuenv.get_wide_pref_path(), "app/" + app_path + "/sce_sys/livearea/contents/" + contents.second);
                else
                    vfs::read_app_file(buffer, emuenv.get_wide_pref_path(), app_path, live_area_path.string() + "/contents/" + contents.second);
                ...
                if (!item.second.empty()) {
                    for (auto &bg_name : item.second["background"]) {
                        if (!bg_name.empty()) {
                            ...
                            if (app_device == VitaIoDevice::vs0)
                                vfs::read_file(VitaIoDevice::vs0, buffer, emuenv.get_wide_pref_path(), "app/" + app_path + "/sce_sys/livearea/contents/" + bg_name);
                            else
                                vfs::read_app_file(buffer, emuenv.get_wide_pref_path(), app_path, live_area_path.string() + "/contents/" + bg_name);
                    ...
                    for (auto &img_name : item.second["image"]) {
                        if (!img_name.empty()) {
                            ...
                            if (app_device == VitaIoDevice::vs0)
                                vfs::read_file(VitaIoDevice::vs0, buffer, emuenv.get_wide_pref_path(), "app/" + app_path + "/sce_sys/livearea/contents/" + img_name);
                            else
                                vfs::read_app_file(buffer, emuenv.get_wide_pref_path(), app_path, live_area_path.string() + "/contents/" + img_name);
...
void browse_live_area_apps_list(GuiState &gui, EmuEnvState &emuenv, const uint32_t button) {
    const auto manual_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/app" / gui.live_area_current_open_apps_list[gui.live_area_app_current_open] / "sce_sys/manual/" };
...
void draw_live_area_screen(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (app_device == VitaIoDevice::ux0) {
        ...
        const auto manual_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/app" / app_path / "sce_sys/manual/" };
