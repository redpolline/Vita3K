...
void init_trophy_collection(GuiState &gui, EmuEnvState &emuenv) {
    const auto TROPHY_PATH{ fs::path(emuenv.get_wide_pref_path()) / "ux0/user" / emuenv.io.user_id / "trophy" };
    ...
    if (fs::exists(TROPHY_CONF_PATH) && !fs::is_empty(TROPHY_CONF_PATH)) {
        for (const auto &trophy : fs::directory_iterator(TROPHY_CONF_PATH)) {
            if (!trophy.path().empty() && fs::is_directory(trophy.path())) {
                ...
                const auto progress_input_file = open_file(emuenv.io, trophy_progress_save_file.c_str(), SCE_O_RDONLY, emuenv.get_wide_pref_path(), "load_trophy_progress_file");
                ...
                for (const auto &group : np_com_list_name_icons) {
                    ...
                    vfs::read_file(VitaIoDevice::ux0, buffer, emuenv.get_wide_pref_path(), "user/" + emuenv.io.user_id + "/trophy/conf/" + np_com_id + "/" + group.second);
...
static void get_trophy_list(GuiState &gui, EmuEnvState &emuenv, const std::string &np_com_id, const std::string &group_id) {
    const auto trophy_conf_id_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/user" / emuenv.io.user_id / "trophy/conf" / np_com_id };
    ...
    for (const auto &trophy : trophy_info) {
        ...
        vfs::read_file(VitaIoDevice::ux0, buffer, emuenv.get_wide_pref_path(), "user/" + emuenv.io.user_id + "/trophy/conf/" + np_com_id + "/" + icon_name);
        if (buffer.empty()) {
