...
static void get_firmware_version(EmuEnvState &emuenv) {
    fs::ifstream versionFile(emuenv.get_wide_pref_path() + L"/PUP_DEC/PUP/version.txt");
    ...
    fs::remove_all(fs::path(emuenv.get_wide_pref_path()) / "PUP_DEC");
}
...
void draw_firmware_install_dialog(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (draw_file_dialog) {
        ...
        if (result == host::dialog::filesystem::Result::SUCCESS) {
            std::thread installation([&emuenv]() {
                install_pup(emuenv.get_wide_pref_path(), pup_path.string(), progress_callback);
                std::lock_guard<std::mutex> lock(install_mutex);
    ...
    if (!finished_installing) {
        ...
        if (ImGui::BeginPopupModal("firmware_installation", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration)) {
            ...
            ImGui::Separator();
            ImGui::Spacing();
            const auto fw_font_package{ fs::path(emuenv.get_wide_pref_path()) / "sa0" };
