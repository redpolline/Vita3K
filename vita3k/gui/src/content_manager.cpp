...
size_t get_app_size(GuiState &gui, EmuEnvState &emuenv, const std::string &app_path) {
    ...
    const auto ADDCONT_PATH{ fs::path(emuenv.get_wide_pref_path()) / "ux0/addcont" / get_app_index(gui, app_path)->title_id };
...
static void get_save_data_list(GuiState &gui, EmuEnvState &emuenv) {
    ...
    fs::path SAVE_PATH{ fs::path{ emuenv.get_wide_pref_path() } / "ux0/user" / emuenv.io.user_id / "savedata" };
...
void init_content_manager(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const auto free_size{ fs::space(emuenv.get_wide_pref_path()).free };
    ...
    const auto query_themes = [&emuenv] {
        const auto THEME_PATH{ fs::path(emuenv.get_wide_pref_path()) / "ux0/theme" };
...
static void get_content_info(GuiState &gui, EmuEnvState &emuenv) {
    const auto APP_PATH{ fs::path(emuenv.get_wide_pref_path()) / "ux0/app" / app_selected };
    if (fs::exists(APP_PATH) && !fs::is_empty(APP_PATH)) {
        gui.app_selector.app_info.size = get_recursive_directory_size(APP_PATH);
    }

    addcont_info.clear();
    const auto ADDCONT_PATH{ fs::path(emuenv.get_wide_pref_path()) / "ux0/addcont" / app_selected };
    if (fs::exists(ADDCONT_PATH) && !fs::is_empty(ADDCONT_PATH)) {
        for (const auto &addcont : fs::directory_iterator(ADDCONT_PATH)) {
            ...
            if (vfs::read_file(VitaIoDevice::ux0, params, emuenv.get_wide_pref_path(), content_path.string() + "/sce_sys/param.sfo")) {
                SfoFile sfo_handle;
    ...
    if (menu.empty()) {
        ...
        if (content_delete) {
            for (const auto &content : contents_selected) {
                if (content.second) {
                    if (menu == "app") {
                        fs::remove_all(fs::path(emuenv.get_wide_pref_path()) / "ux0/app" / content.first);
                        fs::remove_all(fs::path(emuenv.get_wide_pref_path()) / "ux0/addcont" / content.first);
                        gui.app_selector.user_apps.erase(get_app_index(gui, content.first));
                        gui.app_selector.user_apps_icon.erase(content.first);
                    }
                    const auto SAVE_PATH{ fs::path(emuenv.get_wide_pref_path()) / "ux0/user" / emuenv.io.user_id / "savedata" / content.first };
