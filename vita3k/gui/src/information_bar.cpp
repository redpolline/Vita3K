...
static bool init_notice_icon(GuiState &gui, EmuEnvState &emuenv, const fs::path &content_path, const NoticeList &info) {
    ...
    if (!vfs::read_file(VitaIoDevice::ux0, buffer, emuenv.get_wide_pref_path(), content_path)) {
        if (info.type == "trophy") {
            LOG_WARN("Icon no found for trophy id: {} on NpComId: {}", info.content_id, info.id);
            return false;
        } else {
            if (!vfs::read_app_file(buffer, emuenv.get_wide_pref_path(), info.id, "sce_sys/icon0.png")) {
...
static bool set_notice_info(GuiState &gui, EmuEnvState &emuenv, const NoticeList &info) {
    ...
    if (info.type == "content") {
        ...
        if (vfs::read_file(VitaIoDevice::ux0, params, emuenv.get_wide_pref_path(), content_path / "sce_sys/param.sfo")) {
            SfoFile sfo_handle;
        ...
        const auto trophy_conf_id_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/user" / emuenv.io.user_id / "trophy/conf" / info.id };
...
void get_notice_list(EmuEnvState &emuenv) {
    ...
    const auto notice_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/user/notice.xml" };
