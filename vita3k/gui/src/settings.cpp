...
static void get_themes_list(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const auto fw_theme_path{ fs::path(emuenv.get_wide_pref_path()) / "vs0/data/internal/theme" };
        ...
        for (const auto &name : theme_preview_name[theme.first]) {
            if (!name.second.empty()) {
                ...
                if (theme.first == "default")
                    vfs::read_file(VitaIoDevice::vs0, buffer, emuenv.get_wide_pref_path(), name.second);
                else
                    vfs::read_file(VitaIoDevice::ux0, buffer, emuenv.get_wide_pref_path(), fs::path("theme") / string_utils::utf_to_wide(theme.first) / name.second);
                ...
                if (selected.empty()) {
                    ...
                    if (popup.empty()) {
                        ...
                        if (ImGui::Button(common["ok"].c_str(), BUTTON_SIZE) || ImGui::IsKeyPressed(static_cast<ImGuiKey>(emuenv.cfg.keyboard_button_cross))) {
                            ...
                            fs::remove_all(fs::path{ emuenv.get_wide_pref_path() } / "ux0/theme" / string_utils::utf_to_wide(selected));
