...
static bool init_avatar(GuiState &gui, EmuEnvState &emuenv, const std::string &user_id, const std::string avatar_path) {
    const auto avatar_path_wstr = avatar_path == "default" ? fs::path(emuenv.shared_path) / "data/image/icon.png" : fs::path(string_utils::utf_to_wide(avatar_path));
...
void get_users_list(GuiState &gui, EmuEnvState &emuenv) {
    gui.users.clear();
    const auto user_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/user" };
...
void save_user(GuiState &gui, EmuEnvState &emuenv, const std::string &user_id) {
    const auto user_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/user" / user_id };
