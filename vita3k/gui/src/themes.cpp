...
void init_theme_start_background(GuiState &gui, EmuEnvState &emuenv, const std::string &content_id) {
    ...
    if (!content_id.empty() && (content_id != "default")) {
        const auto THEME_PATH_XML{ fs::path(emuenv.get_wide_pref_path()) / "ux0/theme" / content_id_wstr / "theme.xml" };
        pugi::xml_document doc;
    ...
    if (theme_start_name.empty()) {
        ...
        if (fs::exists(fs::path(emuenv.get_wide_pref_path()) / "vs0" / DEFAULT_START_PATH))
            vfs::read_file(VitaIoDevice::vs0, buffer, emuenv.get_wide_pref_path(), DEFAULT_START_PATH);
        else {
            LOG_WARN("Default start background not found, install firmware for fix this.");
            return;
        }
    } else
        vfs::read_file(VitaIoDevice::ux0, buffer, emuenv.get_wide_pref_path(), fs::path("theme") / content_id_wstr / theme_start_name);
...
bool init_theme(GuiState &gui, EmuEnvState &emuenv, const std::string &content_id) {
    ...
    if (content_id != "default") {
        const auto THEME_XML_PATH{ fs::path(emuenv.get_wide_pref_path()) / "ux0/theme" / content_id_wstr / "theme.xml" };
        pugi::xml_document doc;
        ...
        if (doc.load_file(THEME_XML_PATH.c_str())) {
            ...
            if (!info_bar_prop.empty()) {
                ...
                for (const auto &notice : notice_name) {
                    ...
                    vfs::read_file(VitaIoDevice::ux0, buffer, emuenv.get_wide_pref_path(), fs::path("theme") / content_id_wstr / name);
        ...
        for (const auto &bg : app_id_bg_list) {
            if (fs::exists(fs::path(emuenv.get_wide_pref_path()) / "vs0/app" / bg / "sce_sys/pic0.png"))
    ...
    for (const auto &icon : theme_icon_name) {
        ...
        if (name.empty())
            vfs::read_file(VitaIoDevice::vs0, buffer, emuenv.get_wide_pref_path(), "app/" + title_id + "/sce_sys/icon0.png");
        else
            vfs::read_file(VitaIoDevice::ux0, buffer, emuenv.get_wide_pref_path(), fs::path("theme") / content_id_wstr / name);
    ...
    for (const auto &bg : theme_bg_name) {
        ...
        if (content_id == "default")
            vfs::read_file(VitaIoDevice::vs0, buffer, emuenv.get_wide_pref_path(), "app/" + bg + "/sce_sys/pic0.png");
        else
            vfs::read_file(VitaIoDevice::ux0, buffer, emuenv.get_wide_pref_path(), fs::path("theme") / content_id_wstr / bg);
