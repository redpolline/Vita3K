...
EXPORT(int, sceKernelGetStderr) {
    ...
    return open_file(emuenv.io, "tty0:", SCE_O_WRONLY, emuenv.get_wide_pref_path(), export_name);
}

EXPORT(int, sceKernelGetStdin) {
    TRACY_FUNC(sceKernelGetStdin);
    return open_file(emuenv.io, "tty0:", SCE_O_RDONLY, emuenv.get_wide_pref_path(), export_name);
}
