...
EXPORT(int, _sceIoDopen, const char *dir) {
    ...
    return open_dir(emuenv.io, dir, emuenv.get_wide_pref_path(), export_name);
}
...
EXPORT(int, _sceIoDread, const SceUID fd, SceIoDirent *dir) {
    ...
    return read_dir(emuenv.io, fd, dir, emuenv.get_wide_pref_path(), export_name);
}

EXPORT(int, _sceIoGetstat, const char *file, SceIoStat *stat) {
    TRACY_FUNC(_sceIoGetstat, file, stat);
    return stat_file(emuenv.io, file, stat, emuenv.get_wide_pref_path(), export_name);
}
...
EXPORT(int, _sceIoGetstatByFd, const SceUID fd, SceIoStat *stat) {
    ...
    return stat_file_by_fd(emuenv.io, fd, stat, emuenv.get_wide_pref_path(), export_name);
}
...
EXPORT(int, _sceIoMkdir, const char *dir, const SceMode mode) {
    ...
    return create_dir(emuenv.io, dir, mode, emuenv.get_wide_pref_path(), export_name);
}
...
EXPORT(int, _sceIoOpen, const char *file, const int flags, const SceMode mode) {
    ...
    return open_file(emuenv.io, file, flags, emuenv.get_wide_pref_path(), export_name);
}
