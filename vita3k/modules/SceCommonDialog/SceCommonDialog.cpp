...
static void check_empty_param(EmuEnvState &emuenv, const SceAppUtilSaveDataSlotEmptyParam *empty_param, const uint32_t idx) {
    ...
    if (empty_param) {
        ...
        if (iconPath) {
            ...
            vfs::read_file(VitaIoDevice::ux0, thumbnail_buffer, emuenv.get_wide_pref_path(), thumbnail_path);
...
static void check_save_file(const uint32_t index, EmuEnvState &emuenv, const char *export_name) {
    SceUID fd = open_file(emuenv.io, construct_slotparam_path(emuenv.common_dialog.savedata.slot_id[index]).c_str(), SCE_O_RDONLY, emuenv.get_wide_pref_path(), export_name);
    if (fd < 0) {
        ...
        vfs::read_file(device, thumbnail_buffer, emuenv.get_wide_pref_path(), thumbnail_path);
