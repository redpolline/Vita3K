...
bool get_shaders_cache_hashs(State &renderer) {
    const auto shaders_path{ fs::path(renderer.cache_path) / "cache/shaders" / renderer.title_id / renderer.self_name };
    ...
    if (versionInFile != shader::CURRENT_VERSION || features_mask != renderer.get_features_mask()) {
        ...
        fs::remove_all(fs::path(renderer.cache_path) / "shaderlog" / renderer.title_id / renderer.self_name);
...
static bool load_shader(const char *hash, const char *extension, const char *cache_path, const char *title_id, const char *self_name, char **destination, std::size_t &size_read) {
    const auto shader_path = fs_utils::construct_file_name(cache_path, (fs::path("cache/shaders") / title_id / self_name).string().c_str(), hash, extension);
