...
void sync_texture(GLState &state, GLContext &context, MemState &mem, std::size_t index, SceGxmTexture texture,
    const Config &config, const std::string &cache_path, const std::string &title_id) {
    ...
    if (config.dump_textures) {
        ...
        renderer::gl::texture::dump(texture, mem, parameter_name, cache_path, title_id, program_hash);
    }
