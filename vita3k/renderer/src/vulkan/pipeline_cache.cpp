...
void PipelineCache::read_pipeline_cache() {
    const auto shaders_path{ fs::path(state.cache_path) / "cache/shaders" / state.title_id / state.self_name };
...
bool PipelineCache::precompile_shader(const Sha256Hash &hash) {
    const auto shader_path{ fs::path(state.cache_path) / "cache/shaders" / state.title_id / state.self_name };
