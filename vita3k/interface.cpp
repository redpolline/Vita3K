...
static bool is_nonpdrm(EmuEnvState &emuenv, const fs::path &output_path) {
    const auto app_license_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/license" / emuenv.app_info.app_title_id / fmt::format("{}.rif", emuenv.app_info.app_content_id) };
...
bool install_archive_content(EmuEnvState &emuenv, GuiState *gui, const fs::path &archive_path, const ZipPtr &zip, const std::string &content_path, const std::function<void(ArchiveContents)> &progress_callback) {
    ...
    auto output_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0" };
    ...
    if (!copy_path(output_path, emuenv.get_wide_pref_path(), emuenv.app_info.app_title_id, emuenv.app_info.app_category))
        return false;
...
static bool install_content(EmuEnvState &emuenv, GuiState *gui, const fs::path &content_path) {
    ...
    auto dst_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0" };
    ...
    if (!copy_path(dst_path, emuenv.get_wide_pref_path(), emuenv.app_info.app_title_id, emuenv.app_info.app_category))
        return false;
...
static ExitCode load_app_impl(SceUID &main_module_id, EmuEnvState &emuenv, const std::wstring &path) {
    ...
    if (emuenv.cfg.archive_log) {
        const fs::path log_directory{ string_utils::wide_to_utf(emuenv.get_wide_pref_path()) + "/logs" };
    ...
    init_savedata_app_path(emuenv.io, emuenv.get_wide_pref_path());
    ...
    const auto module_app_path{ fs::path(emuenv.get_wide_pref_path()) / "ux0/app" / emuenv.io.app_path / "sce_module" };
